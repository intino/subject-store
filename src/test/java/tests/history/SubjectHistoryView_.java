...
import static systems.intino.datamarts.subjectstore.view.history.format.ColumnDefinition.Type.Categorical;
import static systems.intino.datamarts.subjectstore.view.history.format.ColumnDefinition.Type.Numerical;
...
public class SubjectHistoryView_ {
	...
	private final static String expected = """
		year	month	day-formula	temp-total	temp-average	temp-norm	temp-trend	temp-last	sky-first	sky-count	temp-norm%
	...
	temperature:00	temperature:00-1	temperature:00-2	temperature:00-3	temperature:00+1
	...
	public void should_export_to_tabular_report_with_format_as_object() throws IOException {
		...
		HistoryFormat historyFormat = new HistoryFormat(new RowDefinition(from, to, Duration.ofDays(7)))
			...
			.add("temp-total","temperature:00.sum")
			.add("temp-average","temperature:00.average")
			.add("temp-norm","temp-total", new MinMaxNormalizationFilter())
			.add("temp-trend","temp-average", new RollingAverageFilter(3))
			.add("temp-last","temperature:00.last", Numerical)
			.add("sky-first","sky.first", Categorical)
	...
	public void test_subject_history_view_with_lag_and_lead_filters() throws IOException {
		...
		for (int i = 0; i < 2000; i++) {
			...
			batch.on(from.plus(i, HOURS), "test")
					.put("temperature:00.sau", 20 + 4 * sin(i))
					.terminate();
			batch.terminate();
		}
		...
		SubjectHistoryView.of(history)
				...
				.add("temperature:00", "temperature:00.sau.mean")
				.add("temperature:00-1", "temperature:00", new LagFilter(1))
				.add("temperature:00-2", "temperature:00", new LagFilter(2))
				.add("temperature:00-3", "temperature:00", new LagFilter(3))
				.add("temperature:00+1", "temperature:00", new LeadFilter(1))
	...
	public void should_export_to_tabular_report_with_format_as_string() throws IOException {
		SubjectHistory history = new SubjectHistory("map", connection);
			...
			columns:
			  ...
			  - name: temp-total
			    calc: temperature:00.sum
			
			  - name: temp-average
			    calc: temperature:00.average
